---
title: Psorcast 30s Walk Analysis
...
```{r echo = FALSE}
get_feature_boxplot <- function(data, feature, title, x, y){
    ggplot(data=data, 
       aes_string(
           x = "has_lower_pain", 
           y = feature,
    ...
    geom_boxplot(
        alpha =  0.5,
        width = 0.5/length(unique(data$has_lower_pain))) +
    ...
    dplyr::mutate(
        has_lower_pain = case_when(
            stringr::str_detect(
                ...
                "knee|hip|ankle") ~ "Has Lower Joint Pain", 
            TRUE ~ "No Lower Joint Pain"),
        knee_pain = case_when(
            stringr::str_detect(
                complete_case_joint_pain_list, "knee") ~ "Knee Pain", 
            TRUE ~  "No Lower Joint Pain"),
        hip_pain = case_when(
            stringr::str_detect(
                complete_case_joint_pain_list, "hip") ~ "Hip Pain", 
            TRUE ~  "No Lower Joint Pain"),
        ankle_pain = case_when(
            stringr::str_detect(
                complete_case_joint_pain_list, "ankle") ~ "Ankle Pain", 
            TRUE ~ "No Lower Joint Pain")) %>%
    dplyr::select(
        participantId, age, sex, diagnosis, 
        matches("pain|step|stride|symmetry|cadence"))
```

### Samples of Reported Pain/No-Pain
```{r, echo = FALSE, warning = FALSE, message = FALSE}
pain_no_pain <- cleaned.data %>% 
                 dplyr::group_by(has_lower_pain) %>% 
                 dplyr::summarise(n = n()) %>%
                 dplyr::select(status = has_lower_pain, everything())

gender_break_down <- cleaned.data %>% 
                 dplyr::group_by(sex, has_lower_pain) %>% 
                 dplyr::summarise(n = n()) %>%
                 dplyr::select(sex, status = has_lower_pain, everything())
...
ggplot(data=cleaned.data %>% dplyr::filter(sex != ""), 
       aes(x=has_lower_pain, y = age, fill = sex)) +
...
### Walk 30secs Feature Comparison for Lower Joints Pain/No-Pain
