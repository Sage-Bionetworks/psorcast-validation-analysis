############################################
#' Script to curate MD Joint Counting data from 
...
SCRIPT_PATH <- "curate_tables/MDJointCounting.R"
...
MD_JOINT_COUNTING <- 'syn20700202'
OUTPUT_TBL_NAME <- 'MDJointCounting-Curated'
...
main <- function(){
    ...
    create_table <- 
        query_table(MD_JOINT_COUNTING) %>%
        dplyr::select(participantId = participantID, 
                      everything()) %>%
        copy_file_handles(
            tbl_id = MD_JOINT_COUNTING,
        ...
        regenerate_table(tbl_name = OUTPUT_TBL_NAME,
                         ...
                         file_handle_cols = FILE_HANDLE_COLS,
                         new_cols = NEW_COLS,
                         src_tbl_id = c(MD_JOINT_COUNTING),
                         used = c(MD_JOINT_COUNTING),
                         executed = GIT_URL,
                         name = "curate MD joint counts")
}
