...
main <- function(){
    ...
    joint_summaries <- purrr::map(names(JOINT_TBL_REF), function(activity){
        ...
        #' joint annotation
        if(activity == "dig_jc"){
            joint_annotation = "digital joint count"
        }else if(activity == "gs_jc"){
            joint_annotation = "gold-standard joint count"
        }else{
            joint_annotation = "gold-standard joint swell"
        }
        ...
        entity <- save_to_synapse(data = joint_data,
        ...
        synSetAnnotations(
            ...
            task = joint_annotation)
        
        #' save error log to synapse
        entity <- save_to_synapse(data = error_log,
